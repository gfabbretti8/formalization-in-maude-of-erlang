...
message ($p_2~\ms{!}~hello$ requires to send atom $hello$ to process $p_2$), when the rule is fired the process is consumed to produce the message
...
ongoing work. In Section~\ref{sec:related} we compare with related work. Finally, in Section~\ref{sec:conclusion} we give some
...
process is the pid (pids are integer numbers in our implementation),
...
where $\nil$ is the empty system, $P$ an entity of the lower level and $\op(S_1,\ldots,S_n)$ an $n$-ary operator
...
Second, the rules defining the operational semantics must fit the format in Fig.~\ref{fig:forwardrules}.
...
Then, the forward reversible semantics is defined by decorating the rules of the non-reversible operational semantics as depicted in
...
a fragment of Erlang instead of one of Core Erlang and the use of labels) we were still able to re-use some of their
...
layers: one for expressions and one for systems. This division is quite convenient for the formalization in Maude, as we
can formalize the expression level as an equational theory and then use rewriting
...
forth between the expression level and the system level. Examples of this mechanism are introduced below.
...
\verb+CLSEQ+ of the receive against the payload using the environment \verb+ENV+. If the match succeeds (that is the result of applying \verb+#entityMatch+ matches the pattern \verb+#entityMatchSuccess(EXSEQ | ENV')+) then it returns the selected clause of the receive \verb+EXSEQ+ as well as the environment enriched with the bindings from the match
...
the sequence in $\ms{begin}~e_1,\ldots,e_n~\ms{end}$ and we push on the stack
...
As in~\cite{LaneseM20}, the input semantics must follow a given
format so that the approach can be applied.

Let us describe such format. First, it must include a module named \verb+SEM_SYSTEM+ which defines the system level. As an example, Fig.~\ref{fig:sem-system} depicts the system module for the Erlang
...
To this end, the sorts produced by the lower level operators must extend the sort
\verb+Entity+.
...
The subsort relation \verb+EntityWithKey < Entities+ is added to the module, so that now all system-level
...
The final transformation concerning the \verb+Entities+ module updates the
equations to translate entities between their representation and their meta-representation so to work on entities with keys.
...
notation. Thus, rules must be extended to deal with tagged entities,
...
side of a rule, we first collect all the keys on the left-hand side of
the rule, then we concatenate them together into a new list, say $l$,
and finally we tag each of the new entities with $l$ concatenated with
a different number for each entity.

In practice, when transforming a rule, we match each key on the
left-hand side against a variable, and we use the variables on the
right-hand side to generate the new keys. Furthermore, we create the
required memory.
...
the process is initially tagged with a key \verb_key(N)_, then the new
entities on the right side are tagged with fresh keys \verb_key(0 N)_
and \verb_key( 1N)_, built from \verb_key(N)_. Moreover, the rule also
