...
expression, in fact a naive management could produce unwanted results.
...
More in general, we need to be careful while evaluating statements or functions which produce a
sequence of expressions as they could produce illegal expressions or unwanted effects.

The solution that we adopted consists of wrapping the produced sequence of
...
\]
and in this case $X$ is bound to the result of $Z-M$.

More in general, wrapping a sequence of expressions with erlang parenthesis
resolve unwanted ambiguities.

Doing so requires us to properly manage the environment
corresponding to each block of code. To do so, rather than having a single
environment inside each process, we have a stack of environments, so $\theta$ in reality has the
...
the sequence with $\ms{begin}~e_1,\ldots,e_n~\ms{end}$ and we push on the stack of environments
the appropriate environment to evaluate $e_1,\ldots,e_n$. 
